...
async function updateChecksForCompletedSastScan(run, context, scanConfig) {
  ...
  if (retry === 0 && artifactRequest.data.total_count === 0) {
    ...
    logger.info("Failed to fetch results artifacts.");
    return;
  }
  ...
  for (const artifact of artifacts.artifacts) {
    ...
    const secondUrl = `GET /repos/${workflow_reopo_owner}/${workflow_repo_name}/actions/artifacts/${artifact.id}/zip`;
    const artifactData = await context.octokit.request(secondUrl);
  ...
  if (annotations.length === 0) {
    ...
    logger.info('update success status on check result 1');
    return;
  }
  ...
  for (let index = 0; index < annotations.length / maxNumberOfAnnotations; index++) {
    ...
    if (annotationBatch !== []) {
      ...
      logger.info('update success status on check result');
